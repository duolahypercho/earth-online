...
assert(materialSource.includes("material.customProgramCacheKey = () => 'sf-map-building-palette-v1'"));
assert(materialSource.includes("const LEGACY_BUILDING_PALETTE_ID_V1 = 'c7ad8a-aa765c-77858c-8b6456';"));
assert(materialSource.includes('const LEGACY_BUILDING_GRID_METRES_V1 = 62;'));
assert(materialSource.includes('float sfRoofFacing = smoothstep(0.16, 0.84, abs(normal.y));'));
assert(materialSource.includes('diffuseColor.rgb *= mix(0.72, 1.08, sfRoofFacing);'));
assert(materialSource.includes('const compileLegacy = material.onBeforeCompile;'));
assert(materialSource.includes('sfSourceToneColor / max(sfBoundaryLegacyTone, vec3(0.000001))'));
