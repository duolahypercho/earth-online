...
try {
  ...
  const checks = [
    ...
    ['street width is plausible for the selected source', results.state?.generator === 'sf-builtin'
      ? Number(results.state?.avgStreetWidth || 0) >= 3.5
      : Number(results.state?.avgStreetWidth || 0) >= 11],
