...
assert(source.includes("version: 'sf-map-render-depth-v2'"), 'Presentation diagnostics must expose the current renderer-only depth policy.');
assert(source.includes("waterNormalResponse: 'world-up-view-space-v1'"), 'Presentation diagnostics must expose the reviewed water normal response.');
...
assert(source.includes('function applyWaterPresentation(material)'), 'Water smoothing must remain a runtime material policy, not a source-geometry rewrite.');
assert(source.includes("normal = normalize(mat3(viewMatrix) * vec3(0.0, 1.0, 0.0));"), 'Water lighting must use a deterministic world-up normal transformed into view space.');
assert(source.includes("material.customProgramCacheKey = () => 'sf-map-water-world-up-v1'"), 'Water shader programs must use the reviewed deterministic cache identity.');
assert(source.includes("material.userData.sfMapWaterNormalResponse = 'world-up-view-space-v1'"), 'Shared water materials must be patched at most once.');
assert((source.match(/applyWaterPresentation\(node\.material\);/g) || []).length === 1, 'Only the water material may receive the world-up normal response.');
assert(source.includes("water: 'sf-map-water-world-up-v1'"), 'Runtime diagnostics must expose the water shader cache identity.');
