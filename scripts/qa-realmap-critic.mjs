...
lines.push('4. **Real-map interiors are generic generated rooms.** Enterable OSM buildings now work and return to the street, but rooms use one simple template rather than authored variants or resident schedules. The authored main game still has richer interiors and NPC routines.');
lines.push('');
...
lines.push('3. **Interior depth.** Add authored room variants (cafe, office, rowhouse) and resident schedules inside real-map buildings. Acceptance: at least one real OSM building per Downtown block can be entered with a return path, and at least three room archetypes are visually distinguishable.');
