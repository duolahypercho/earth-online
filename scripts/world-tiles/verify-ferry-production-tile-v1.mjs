...
function inspectPrimitive(gltf, bin, primitive) {
  ...
  for (let axis = 0; axis < 3; axis += 1) { assert(Math.abs(min[axis] - positionAccessor.min[axis]) <= 5e-6, `${primitive.extras.category} accessor min mismatch`); assert(Math.abs(max[axis] - positionAccessor.max[axis]) <= 5e-6, `${primitive.extras.category} accessor max mismatch`); }
...
assert.equal(receipt.ferryBuilding.present, true); assert.equal(receipt.ferryBuilding.sourceFeatureId, 'way/558731934'); assert.match(receipt.relationCoverage.statement, /not represented or claimed/);
for (const lod of receipt.lods) {
  ...
  assert.equal(gltf.meshes.length, 1); assert.equal(gltf.meshes[0].primitives.length, 3); const stats = Object.fromEntries(gltf.meshes[0].primitives.map((primitive) => [primitive.extras.category, inspectPrimitive(gltf, bin, primitive)]));
  for (const category of ['terrain', 'roads', 'buildings']) { const { min: _min, max: _max, ...actual } = stats[category]; assert.deepEqual(actual, lod.meshStats[category], `${category} mesh stats mismatch`); }
